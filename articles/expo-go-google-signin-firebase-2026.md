---
title: "【2026年版】Expo Go で Google サインイン + Firebase Auth を動かす方法"
emoji: "🔐"
type: "tech"
...
Google サインインの情報を調べると、だいたい以下の3パターンが出てきます。
...
ポイントは、iOS クライアント ID を使うことで **reversed client ID スキームのリダイレクト** が使えるという点です。`WebBrowser.openAuthSessionAsync` は内部で `ASWebAuthenticationSession` を使っており、Expo Go であってもカスタムスキームへのリダイレクトをキャッチできます。
...
`WebBrowser.openAuthSessionAsync` は iOS では `ASWebAuthenticationSession` を使います。このAPIは、指定したスキームへのリダイレクトを**アプリにURL Schemeが登録されていなくてもキャッチ**できます。

つまり Expo Go のように URL Scheme を自由に追加できない環境でも、`reversed client ID` スキームへのリダイレクトを受け取れるわけです。
...
Expo Go での開発が終わったら、本番では `@react-native-google-signin/google-signin` に切り替えるのが安定します。ネイティブの Google サインインダイアログが使えるので、UX も良くなります。

この記事のアプローチはあくまで **Expo Go での開発フェーズを快適にする** ためのものです。

## おわりに

正直、ここにたどり着くまでに `invalid_request`、`redirect_uri_mismatch`、プロキシのエラーを何十回と見ました。Expo Go + Google サインインの組み合わせは公式ドキュメントも情報が散らばっていて、かなり厳しい。

同じところでハマっている人の助けになれば幸いです。
