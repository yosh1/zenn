---
title: "GitHub Copilot の Agent Mode・CLI・Coding Agent を使い分けて、開発の進め方を組み直した話"
emoji: "🤖"
type: "tech"
topics: ["GitHubCopilot", "GitHub", "VSCode", "AI", "開発効率化"]
published: false
---
...
Agent Mode が VS Code と JetBrains の両方で GA になり、Copilot CLI はターミナル上のエージェント環境に進化し、Coding Agent は Issue を渡すだけで PR を作ってくる。便利そうなのはわかるけど、どこから手をつけるか迷う——そんな状態だった。

実際にプロジェクトで3つを使い分けてみて、自分なりの使い所が見えてきたので整理してみる。
...
| **Copilot CLI** | ターミナル | 自然言語でタスクを指示、ターミナル上で完結するエージェント環境 | スクリプト作成、環境構築、コマンド操作 |
...
Agent Mode は IDE での対話的な開発に、CLI はターミナル作業の自動化に、Coding Agent は明確なタスクの非同期処理に。使い分けを意識するだけで、開発の進め方がだいぶ変わった。

3つを組み合わせて自分なりのワークフローを組み立てていくのが、Copilot を使いこなす面白さだと思う。まずは普段のタスクに「どのエージェント機能が向いてるか」を考えるところから試してみてほしい。
